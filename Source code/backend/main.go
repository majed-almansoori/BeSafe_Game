...
_, err = db.Exec("UPDATE users SET knowledge_score = $1 WHERE username = $2", reqBody.Score, reqBody.Username)
if err != nil {
	w.WriteHeader(http.StatusInternalServerError)
...
_, err = db.Exec("UPDATE users SET chapter_durations = $1 WHERE username = $2", pq.Array(reqBody.Durations), reqBody.Username)
if err != nil {
	w.WriteHeader(http.StatusInternalServerError)
...
queryStr := fmt.Sprintf("UPDATE users SET %s_counter = $1 WHERE username = $2", featureName)
_, err = db.Exec(queryStr, reqBody.Counter, reqBody.Username)
if err != nil {
	w.WriteHeader(http.StatusInternalServerError)
...
_, err = db.Exec(queryStr, pq.Array(reqBody.Timestamps), reqBody.Username)
if err != nil {
	w.WriteHeader(http.StatusInternalServerError)
